...
contract FarmMulticall {
    ...
    function gaugeCardData(
        address plugin,
        address account
    ) public view returns (GaugeCard memory gaugeCard) {
        ...
        gaugeCard.priceOTOKEN =
            (ITOKEN(TOKEN).getOTokenPrice() * (gaugeCard.priceBase)) /
            1e18;
        gaugeCard.votingWeight = (
            IVoter(voter).totalWeight() == 0
                ? 0
                : (100 * IVoter(voter).weights(plugin) * 1e18) /
                    IVoter(voter).totalWeight()
        );
        gaugeCard.totalSupply = IGauge(gaugeCard.gauge).totalSupply();

        gaugeCard.accountTokenBalance = (
            account == address(0)
                ? 0
                : IERC20(gaugeCard.token).balanceOf(account)
        );
        gaugeCard.accountStakedBalance = (
            account == address(0) ? 0 : IPlugin(plugin).balanceOf(account)
        );

        gaugeCard.rewardTokens = IGauge(gaugeCard.gauge).getRewardTokens();

        uint8[] memory _rewardTokenDecimals = new uint8[](
            gaugeCard.rewardTokens.length
        );
        for (uint i = 0; i < gaugeCard.rewardTokens.length; i++) {
            _rewardTokenDecimals[i] = IERC20Metadata(gaugeCard.rewardTokens[i])
                .decimals();
        }
        gaugeCard.rewardTokenDecimals = _rewardTokenDecimals;

        uint[] memory _rewardsPerToken = new uint[](
            gaugeCard.rewardTokens.length
        );
        for (uint i = 0; i < gaugeCard.rewardTokens.length; i++) {
            _rewardsPerToken[i] = (
                IGauge(gaugeCard.gauge).totalSupply() == 0
                    ? 0
                    : (IGauge(gaugeCard.gauge).getRewardForDuration(
                        gaugeCard.rewardTokens[i]
                    ) * 1e18) / IGauge(gaugeCard.gauge).totalSupply()
            );
        }
        gaugeCard.rewardsPerToken = _rewardsPerToken;

        uint[] memory _accountRewardsEarned = new uint[](
            gaugeCard.rewardTokens.length
        );
        for (uint i = 0; i < gaugeCard.rewardTokens.length; i++) {
            _accountRewardsEarned[i] = (
                account == address(0)
                    ? 0
                    : IGauge(gaugeCard.gauge).earned(
                        account,
                        gaugeCard.rewardTokens[i]
                    )
            );
    ...
    function getGaugeCards(
        uint256 start,
        uint256 stop,
        address account
    ) external view returns (GaugeCard[] memory) {
